...
class DeepSeekAdapter(ProviderAdapter):
    """Adapter for the DeepSeek Platform API provider."""
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                for item in models_list:
                    ...
                    # DeepSeek does not support vision models in its current public API
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    return BenchmarkResult(
                        ...
                        error=(
                            f"DeepSeek API benchmark failed "
    ...
    async def chat(  # type: ignore[override, misc]
        ...
        if not api_key:
            raise ValueError("API key is required for DeepSeek chat completions.")
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code >= 500:
                    return HealthStatus(
                        ...
                        latency_ms=latency_ms,
                        detail=f"DeepSeek API returned server error: {response.status_code}",
                    )
                return HealthStatus(
                    healthy=True,
                    checked_at=datetime.now(UTC),
                    latency_ms=latency_ms,
                    detail=f"DeepSeek API reachable. Status: {response.status_code}",
                )
        except Exception as exc:
            latency_ms = (time.perf_counter() - start_time) * 1000
            return HealthStatus(
                ...
                checked_at=datetime.now(UTC),
                latency_ms=latency_ms,
                detail=f"DeepSeek API unreachable: {exc}",
            )
    ...
    def _guess_context_length(self, model_id: str, raw_metadata: dict[str, Any]) -> int | None:
        ...
        if "deepseek-chat" in model_id_lower:
            return 128000
        if "deepseek-reasoner" in model_id_lower:
            return 128000
