...
def adapter() -> GroqAdapter:
    return GroqAdapter(base_url="https://api.groq.com/openai/v1")


async def test_validate_key_success(adapter: GroqAdapter) -> None:
...
async def test_validate_key_invalid(adapter: GroqAdapter) -> None:
...
async def test_validate_key_exception(adapter: GroqAdapter) -> None:
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
...
async def test_discover_models_success(adapter: GroqAdapter) -> None:
    ...
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        ...
        # First model (Llama-3.3)
        assert models[0].model_id == "llama-3.3-70b-versatile"
        assert models[0].display_name == "Llama 3.3 70b Versatile"
        assert models[0].supports_vision is False
        assert models[0].context_length == 131072

        # Second model (Vision)
        assert models[1].model_id == "llama-3.2-11b-vision-preview"
...
async def test_discover_models_error_or_empty(adapter: GroqAdapter) -> None:
...
async def test_benchmark_success(adapter: GroqAdapter) -> None:
    mock_response = httpx.Response(200, json={"choices": [{"message": {"content": "OK"}}]})
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        mock_post.return_value = mock_response
        result = await adapter.benchmark("llama-3.3-70b-versatile", "valid_key")
...
async def test_benchmark_failure(adapter: GroqAdapter) -> None:
...
async def test_benchmark_timeout_or_exception(adapter: GroqAdapter) -> None:
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
...
async def test_chat_success_stream(adapter: GroqAdapter) -> None:
    messages = [
    ...
    with patch("httpx.AsyncClient.stream") as mock_stream_method:
        ...
        chunks = []
        async for chunk in adapter.chat(messages, "llama-3.3-70b-versatile", api_key="valid_key"):
            chunks.append(chunk)
...
async def test_chat_error_status(adapter: GroqAdapter) -> None:
    ...
    with patch("httpx.AsyncClient.stream") as mock_stream_method:
        ...
        with pytest.raises(httpx.HTTPStatusError) as exc_info:
            async for _ in adapter.chat(messages, "llama-3.3-70b-versatile", api_key="valid_key"):
                pass
        assert exc_info.value.response.status_code == 429
        assert "Rate limit exceeded" in str(exc_info.value)


async def test_chat_missing_key(adapter: GroqAdapter) -> None:
    messages = [Message(role="user", content="Hi")]
    with pytest.raises(ValueError) as exc_info:
        async for _ in adapter.chat(messages, "llama-3.3-70b-versatile"):
            pass
    assert "api key is required" in str(exc_info.value).lower()


def test_remaining_quota(adapter: GroqAdapter) -> None:
    ...
    assert status1.remaining == 4000
    assert status1.limit == 5000
    ...
    assert status3.limit == 14400
    assert status3.remaining == 9400


async def test_generate_image_not_implemented(adapter: GroqAdapter) -> None:
    with pytest.raises(NotImplementedError):
        await adapter.generate_image("A futuristic city")


async def test_health_check_healthy(adapter: GroqAdapter) -> None:
...
async def test_health_check_unhealthy(adapter: GroqAdapter) -> None:
    # Server error 500
